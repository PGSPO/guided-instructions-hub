...
interface MetricCardProps {
  ...
  onClick: (status: IncidentStatus) => void;
  isActive: boolean;
}
...
interface MetricCardsProps {
  metrics: Array<{ title: string; count: number; status: IncidentStatus }>;
  onFilterChange: (status: IncidentStatus | null) => void;
  activeFilter: IncidentStatus | null;
}

export const MetricCards = ({ metrics, onFilterChange, activeFilter }: MetricCardsProps) => {
  const handleCardClick = (status: IncidentStatus) => {
